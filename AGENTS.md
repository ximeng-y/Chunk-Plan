...
| 维度 | 维度独立计费（issue #3）：独立模式下每维度独立四档额度线（12 值/维度），维度动态读自 `getAllLevels()`（含 mod 注册维度）；启用独立模式强制每个 live 维度先配置合法落地坐标（/tp 数据规范，GUI 保存按钮灰禁用、命令拒绝并列出缺失）；耗尽重定向固定 首选/次选/备选 3 槽位 → 其余维度字典序 → 全不可进才封禁；计费开关两种模式通用（关=不计费自由探索）；模式切换不迁移消费（全局桶与维度桶并存）；玩家预设优先于维度额度线；`check`/用量页独立模式只看单维度（用量页顶部下拉可切换查看） |
