...
本模组采用 [Mozilla Public License 2.0](https://www.mozilla.org/en-US/MPL/2.0/)（MPL-2.0）许可开源，完整条款见仓库根目录 [LICENSE](LICENSE) 文件。

- **允许**：自由使用、复制、修改与分发本模组，包括用于您的整合包与服务器，无需申请许可；
- **要求**：修改后的源文件必须继续以 MPL-2.0 许可公开，并保留原版权与许可声明；
- **说明**：MPL-2.0 为文件级许可——被修改的文件须开源；独立新增、不包含本模组代码的文件可另行选择许可；
- **不要求**：开源您整合包中的其他模组或其他独立项目。

Copyright © 2026 ximeng-y
