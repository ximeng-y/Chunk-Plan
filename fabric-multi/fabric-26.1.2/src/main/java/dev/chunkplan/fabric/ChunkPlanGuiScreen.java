...
public final class ChunkPlanGuiScreen extends Screen {
    ...
    private void renderDimensions(GuiGraphicsExtractor g) {
        ...
        g.text(font, Component.translatable("gui.chunkplan.dim.dim_header"), x + 8, 58, COL_GRAY);
        ...
        g.text(font, Component.translatable("gui.chunkplan.dim.usage_header"), x + 420, 58, COL_GRAY);
        // 行内容（控件之外的文字）
        int visibleRows = dimVisibleRows();
        for (int i = 0; i < dims.size(); i++) {
            if (i < dimScroll || i >= dimScroll + visibleRows) {
            ...
            if (!live) {
                ...
                if (!usage.isEmpty()) {
                    g.text(font, Component.literal(font.plainSubstrByWidth(usage, tailW)),
        ...
        int saveY = dimSaveY();
        if (dimCoordsDirty()) {
            g.text(font, Component.translatable("gui.chunkplan.dim.coords_dirty"), x + 248, saveY + 6, COL_RED);
        } else if (dimCoordsSavedShown) {
            g.text(font, Component.translatable("gui.chunkplan.dim.coords_saved"), x + 248, saveY + 6, COL_GRAY);
        } else {
            // 坐标是模式无关的单份存储：两种模式下都可改可清（避免"共享模式改不掉"）
            g.text(font, Component.translatable("gui.chunkplan.dim.spawn_hint"), x + 248, saveY + 6, COL_GRAY);
        }
        if (effIndependent && !missingSpawnDims().isEmpty()) {
            g.text(font, Component.translatable("gui.chunkplan.dim.coords_missing"), x + 248, saveY + 20, COL_RED);
        }
        // 重定向开启但三槽全空：说明回退顺序（槽位条右侧灰字，条目最长处仍留余量）
        if (independent && status.dimConfig().redirectOnExhaust() && redirectOrderAllEmpty()) {
            g.text(font, Component.translatable("gui.chunkplan.dim.redirect_fallback_hint"),
                    x + 620, 42, COL_GRAY);
        }
        ...
        g.text(font, Component.translatable("gui.chunkplan.dim.editing", selectedDim == null ? "—" : selectedDim),
                x + 156, et + 6, COL_TEXT);
        if (!independent) {
            g.text(font, Component.translatable("gui.chunkplan.dim.shared_mode_hint"), x + 300, et + 6, COL_GRAY);
        }
        ...
        if (st != null) {
            ...
            if (anyDirty) {
                ...
                g.text(font, Component.translatable("gui.chunkplan.saved"), x + 224, dimTierHintY(), COL_GRAY);
            }
        }
    }
     ...
     * （黑底 + 白描边 + z 抬层，坑 #51/#52）。
     */
    private void renderDimUsageTip(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            g.text(font, lines.get(i), bx + 6, ty, lineColors.get(i));
            ty += font.lineHeight;
        }
    }
     ...
     */
    private void renderControlHoverHint(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        Component hint = null;
    ...
    private void drawDialogButton(GuiGraphicsExtractor g, int x, int y, int w, int h, Component label, int fillColor) {
    ...
    public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean bl) {
        ...
        // 管理页：重置层级手绘下拉条
        if (page == 1 && isAdmin() && inRect(event.x(), event.y(), resetTierRect)) {
            openResetTierDropdown();
            return true;
        }
        // 管理页：预设选择条 /「按玩家应用」预设选择条（手绘下拉条，展开真下拉）
        if (page == 1 && isAdmin() && inRect(event.x(), event.y(), presetSelectRect)) {
            openPresetDropdown();
            return true;
        }
        if (page == 1 && isAdmin() && inRect(event.x(), event.y(), assignSelectRect)) {
            openAssignDropdown();
            return true;
        }
     ...
     */
    private void renderFeedback(GuiGraphicsExtractor g) {
        if (page != 1 && page != 2) {
            return;
        }
        ...
        g.text(font, Component.literal(shown), x + 5, y + 3, feedbackColor());
    }
