...
public final class ChunkPlanMessages {
    ...
    public static Component quotaAlertMessage(QuotaEngine.WindowAlert alert, boolean zh) {
        ...
        return Component.literal(sb.toString()).append(Component.literal("[" + link + "]")
                .withStyle(s -> s.withColor(ChatFormatting.YELLOW)
                        .withClickEvent(new ClickEvent.RunCommand("/chunkplan rules"))
    ...
    public static Component confirmLink(boolean zh) {
        ...
        return Component.literal("[" + link + "]")
                .withStyle(s -> s.withColor(ChatFormatting.YELLOW)
                        .withClickEvent(new ClickEvent.RunCommand("/chunkplan confirm"))
                        .withHoverEvent(new HoverEvent.ShowText(Component.literal(link))));
    }
    ...
    public static String helpMessage(boolean zh) {
        if (zh) {
            ...
            return "§e--- ChunkPlan 管理员帮助 ---\n"
                    ...
                    + "§f预设：§a/chunkplan preset list | save <名称> [12 值] | delete <名称>§f，save 只写名称即把当前配置存为预设；写 13 个词（开关 窗口 上限 × 4 档）则按指定 12 值保存，不影响全局配置（存储在服务端）\n"
        ...
        return "§e--- ChunkPlan Admin Help ---\n"
                ...
                + "§fPresets: §a/chunkplan preset list | save <name> [12 values] | delete <name>§f - save with a name alone stores the current config; with 12 values (enabled window limit x 4 tiers) it stores them without touching the global config (stored server-side)\n"
