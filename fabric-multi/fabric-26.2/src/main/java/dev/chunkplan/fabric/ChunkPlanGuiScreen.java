...
public final class ChunkPlanGuiScreen extends Screen {
    ...
    private void renderAdmin(GuiGraphicsExtractor g) {
        ...
        int hintY = 36 + 4 * 26 - 4;
        if (dirtyTier) {
            g.text(font, Component.translatable("gui.chunkplan.unsaved"), x, hintY, COL_RED);
        } else if (savedShown) {
            g.text(font, Component.translatable("gui.chunkplan.saved"), x, hintY, COL_GRAY);
        }
        int gy = 36 + 4 * 26 + 6;
    ...
    public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean bl) {
        ...
        if (!resetSuggestions.isEmpty()) {
            for (int i = 0; i < resetSuggestions.size(); i++) {
                if (inRect(event.x(), event.y(), resetTargetX, resetTargetY + resetTargetH + 2 + i * SUGGEST_ROW_H,
