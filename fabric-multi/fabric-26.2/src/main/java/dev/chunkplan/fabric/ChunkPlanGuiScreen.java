...
public final class ChunkPlanGuiScreen extends Screen {
    ...
    private void renderDimensions(GuiGraphicsExtractor g) {
        ...
        if (st != null) {
            for (int i = 0; i < 4; i++) {
                ...
                if (st.dirty[i]) {
                    g.text(font, Component.translatable("gui.chunkplan.unsaved"), x + 272, ry + 6, COL_RED);
                } else if (st.savedShown[i]) {
                    g.text(font, Component.translatable("gui.chunkplan.saved"), x + 272, ry + 6, COL_GRAY);
                }
            }
        }
    }
    ...
    private void renderDimDropdown(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            g.text(font, Component.literal(font.plainSubstrByWidth(dims.get(i), sW - 12)),
