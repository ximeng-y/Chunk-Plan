...
public final class ChunkPlanGuiScreen extends Screen {
    ...
    private void renderUsage(GuiGraphicsExtractor g) {
        ...
        int x = 12;
        int y = 40;
        g.text(font, t(zh, "探索额度状态", "Exploration Quota"), x, y, COL_ACCENT);
        y += 16;
        if (waiting) {
            if (System.currentTimeMillis() - requestTimeMillis > REQUEST_TIMEOUT_MILLIS) {
                g.text(font, t(zh, "未检测到服务器 ChunkPlan（或版本不匹配），命令仍可用",
                        "No server ChunkPlan detected (or version mismatch); commands still work"), x, y, COL_RED);
            } else {
                g.text(font, t(zh, "正在获取额度…", "Fetching quota..."), x, y, COL_GRAY);
            }
            return;
        }
        GuiStatus s = status;
        if (s == null) {
            g.text(font, t(zh, "无法解析服务器返回的状态（可能版本不匹配）", "Could not parse server status (possible version mismatch)"), x, y, COL_RED);
            return;
        }
        if (s.isExempt()) {
        ...
        if (s.lines().isEmpty()) {
            g.text(font, t(zh, "所有探索窗口均已关闭，当前无额度限制", "All quota windows are disabled; no quota limits in effect"),
                    x, y, COL_GREEN);
        } else {
            for (QuotaEngine.LineStatus l : s.lines()) {
                ...
                g.text(font, label + "  " + String.format("%.1f / %.1f", l.spent(), l.limit()), x, y, COL_TEXT);
                y += 12;
                int barW = Math.max(80, Math.min(260, width - 40));
                drawBar(g, x, y, barW, BAR_H, pct, color);
                g.text(font, String.format("%.0f%%", pct), x + barW + 4, y, color);
                y += BAR_H + 2;
                if (l.nextResetMillis() > 0) {
                    g.text(font, t(zh, "下次重置：", "Next reset: ") + ChunkPlanMessages.formatTime(l.nextResetMillis()),
                            x + 12, y, COL_GRAY);
                }
                y += 12;
            }
            y += 2;
            if (s.allExceeded()) {
                g.text(font, t(zh, "已耗尽，预计 ", "Exhausted, recovers at ") + ChunkPlanMessages.formatTime(s.recoveryMillis()),
                        x, y, COL_RED);
            } else {
        ...
        y += 18;
        g.text(font, t(zh, "计费规则", "Billing rules"), x, y, COL_ACCENT);
        y += 12;
        g.text(font, t(zh, "新踏足区块消耗 ", "New chunk costs ") + String.valueOf(s.firstEntryFee()), x + 8, y, COL_GRAY);
        y += 11;
        g.text(font, t(zh, "已踏足区块消耗 ", "Explored chunk costs ") + String.valueOf(s.familiarEntryFee()), x + 8, y, COL_GRAY);
        y += 11;
        g.text(font, t(zh, "高速移动倍率 ", "High-speed multiplier ") + String.valueOf(s.highSpeedMultiplier()) + "x",
                x + 8, y, COL_GRAY);
    }
    ...
    private void renderAdmin(GuiGraphicsExtractor g) {
        ...
        int x = 12;
        if (!isAdmin()) {
            g.text(font, t(zh, "无权限访问管理页", "No permission to view admin page"), x, 44, COL_RED);
            return;
        }
        for (int i = 0; i < 4; i++) {
            int ry = 36 + i * 26;
            g.text(font, "tier" + (i + 1), x, ry + 6, COL_TEXT);
        }
        int gy = 36 + 4 * 26 + 6;
        g.text(font, t(zh, "全部窗口", "All windows"), x, gy + 6, COL_TEXT);
        g.text(font, t(zh, "高速倍率", "Speed ×"), x, gy + 34, COL_TEXT);
        g.text(font, t(zh, "重置额度", "Reset quota"), x, gy + 90, COL_TEXT);
    }
    ...
    private void renderConfirm(GuiGraphicsExtractor g) {
        ...
        g.text(font, t(zh, "确认操作", "Confirm action"), bx + 8, by + 8, COL_ACCENT);
        g.text(font, confirmText == null ? "" : confirmText, bx + 8, by + 28, COL_TEXT);
        int btnY = by + h - 24;
        noX = bx + w - 118;
        noY = btnY;
        noW = 52;
        ...
        g.centeredText(font, t(zh, "取消", "Cancel"), noX + noW / 2, noY + 5, COL_TEXT);
        yesX = bx + w - 60;
        yesY = btnY;
        yesW = 52;
        yesH = 18;
        g.fill(yesX, yesY, yesX + yesW, yesY + yesH, 0xFF2E7D32);
        g.centeredText(font, t(zh, "确认", "Yes"), yesX + yesW / 2, yesY + 5, COL_TEXT);
    }
