...
public final class ChunkPlanGuiScreen extends Screen {
    ...
    private void renderAdmin(GuiGraphicsExtractor g) {
        ...
        g.text(font, Component.translatable("gui.chunkplan.preset_title"), x, gy + 174, COL_TEXT);
        g.text(font, Component.translatable("gui.chunkplan.preset_save_label"), x, gy + 202, COL_TEXT);
        g.text(font, Component.translatable("gui.chunkplan.preset_assign_label"), x, gy + 230, COL_TEXT);
        if (resetSuggestions.isEmpty()) { // 下拉弹出期间提示行被遮挡，收起后恢复（移至重置行右侧，下方为预设区）
            g.text(font, Component.translatable("gui.chunkplan.reset_hint"), x + 296, gy + 152, COL_GRAY);
        }
    }
